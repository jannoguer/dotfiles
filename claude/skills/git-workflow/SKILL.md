...
| `style` | Formatting, no code change | `style: fix indentation in login` |
...
# BAD: Has a body (not allowed)
git commit -m "fix(api): retry on 503

Added exponential backoff with max 3 attempts."

# GOOD: Single line, specific, under 50 chars
git commit -m "fix(api): retry requests on 503"
git commit -m "feat(auth): add OAuth2 login flow"
git commit -m "chore(deps): bump eslint to v9"
```
...
## LLM Identity in Git Operations

The LLM must never include itself in any git operation. This means:

- No `Co-Authored-By: Claude` or any AI attribution in commit messages
- No AI-generated signatures, trailers, or footers on commits
- No AI attribution in PR descriptions, tags, or release notes
- No reference to Claude, Anthropic, or any AI tool in pushed content

Commits must reflect only the human author(s) as configured in git config.
